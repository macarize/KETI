...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, H, W, self.num_features).permute(0, 3, 1, 2).contiguous()
        return x
...
def load_pretrained(model, model_path):
    ...
    state_dict = {key: value for key, value in state_dict.items() if not key.startswith('head') and 'attn_mask' not in key}
...
def Swin_B(pretrain='/mnt/data1/jiajian/code/checkpoints/swin_base_patch4_window7_224.pth'):
    return SwinTransformer(pretrain=pretrain, embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32],)
