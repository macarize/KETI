...
pred = np.loadtxt('KETI_Person_resnet_01.csv', delimiter=',', dtype=float)
gt = np.loadtxt('data/KETI/test.csv', delimiter=',', dtype=float)


fl = open('attr_id.json', 'r')
attr_id = json.load(fl)
print(attr_id)
fl = open('KETI_01_index_attr.json', 'r')
attr_id_part = json.load(fl)
print(attr_id_part)

labels = []
for i in range(len(pred)):
    label = np.zeros(149)
    indices = np.where(pred[i] > 0.5)[0]
    print(indices)
    for k, item in enumerate(indices):
        temp = attr_id_part[str(item)]
        print(temp)
        attr = attr_id[str(temp)]
        label[attr] = 1
    labels.append(label)

labels = np.stack(labels, axis=0)
np.savetxt('part_to_all_01.csv', labels, delimiter=',')
...
train_result = get_pedestrian_metrics(gt, labels, index=None)


print('ma: {:.4f}, label_f1: {:.4f}, pos_recall: {:.4f} , neg_recall: {:.4f} \n'.format(
